...
public class BinaryTree<E> {
	...
	/*
	9. Write a binary tree method that prints to System.out one line for each node in the tree, with each
	line of the form
	parent [, child1 [, child2]]
	where the brackets indicate things that may or may not be there (it may have 0, 1, or 2 children).
	 */
	public void nodeLine(BinaryTree<E> node) {
		String line = node.getData().toString();
		if (hasLeft()) line += (", "+left.data.toString());
		if (hasRight()) line += (", "+right.data.toString());
		System.out.println(line);

		if (hasLeft()) left.nodeLine(left);
		if (hasLeft()) right.nodeLine(right);
	}
	 ...
	 *
	 * This produces a "shallow" copy of a tree, down to the given depth. By "shallow", I mean that the tree structure
	 * is copied, but the same data elements are used (not copies of them). By "down to the given depth", I mean that
	 * nodes deeper than the value are chopped out. For depth 0, only the node itself is kept; its children are null.
	 * For depth 1, the children are kept but their children are null.
	 */
	public BinaryTree<E> copyToDepth(int d) {
		if (d == 0) {
			return new BinaryTree<E>(data);
		}
		BinaryTree<E> newL = new BinaryTree<E>(null);
		BinaryTree<E> newR = new BinaryTree<E>(null);

		if (hasLeft()) newL = left.copyToDepth(d - 1);
		if (hasRight()) newR = right.copyToDepth(d - 1);
		return new BinaryTree<E>(data, newL, newR);
	}
