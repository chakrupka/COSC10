 ...
 * Finished draw() and findColliders() methods
 */
public class CollisionGUI extends DrawingGUI {
	...
	public void draw(Graphics g) {
		...
		for (Blob b : blobs) {
			if (colliders != null && (colliders.contains(b))) {
				g.setColor(Color.RED); // Ask the colliders to draw themselves in red.
				b.draw(g);
			}
			else {
	...
	private void findColliders() {
		// Create the tree
		PointQuadtree<Blob> tree = new PointQuadtree<>(blobs.get(0), 0, 0, 800, 600);
		...
		for (Blob b : blobs) {
			colliders.addAll(tree.findInCircle(b.getX(), b.getY(), b.getR()*2));
			colliders.remove(b);
		}
	}
