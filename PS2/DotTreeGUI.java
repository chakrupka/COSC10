 ...
 * Finished handleMousePress(), drawTree() and handleKeyPress() methods
 */
public class DotTreeGUI extends DrawingGUI {
	private static final int width=800, height=600;		// size of the universe
	private static final int dotRadius = 5;				// to draw dot, so it's visible
	private static final Color[] rainbow = {Color.RED, Color.ORANGE, Color.YELLOW, Color.GREEN,
			Color.BLUE, Color.MAGENTA}; 				// to color different levels differently
	...
	private int testFind(int x, int y, int r, int expectedCircleRectangle, int expectedInCircle, int expectedHits) {
		...
		if (Geometry.getNumCircleRectangleTests() != expectedCircleRectangle) {
			errs++;
			System.err.println(which+": wrong # circle-rectangle, got "+Geometry.getNumCircleRectangleTests()+
					" but expected "+expectedCircleRectangle);
		}
		if (Geometry.getNumInCircleTests() != expectedInCircle) {
			errs++;
			System.err.println(which+": wrong # in circle, got "+Geometry.getNumInCircleTests()+
					" but expected "+expectedInCircle);
		}
		if (num != expectedHits) {
			errs++;
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400, 300), 0, 0, 800, 600); // start with A
		tree.insert(new Dot(200, 150)); // B
		tree.insert(new Dot(100, 75)); // C
		tree.insert(new Dot(50, 38)); // D

		int bad = 0;
		// rect for A [B [C [D]]]; circle for A, B, C, D; find A, B, C
		bad += testFind(200,150,249,4,4,3);
		// rect for A [B [C [D]]]; circle for A, B, C, D; find A, B, C, D
		bad += testFind(200,150,251,4,4,4);
		// rect for A [B [C]]; circle for A, B; find A
 		bad += testFind(400,300,15,3,2,1);
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		g.fillOval((int)tree.getPoint().getX()-dotRadius, (int)tree.getPoint().getY()-dotRadius,
